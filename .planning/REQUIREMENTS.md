...
- [ ] **CAP-01** (C1): User can select a native **Anthropic Messages API** backend with streaming, tool-use, prompt caching, and token counting — reusing OkHttp + `MontoyaHttpTransport` (no SDK that bypasses Burp), API key encrypted via SEC-01 *[Phase 14 scope decision, 2026-06-10: ships streaming (single-chunk, proxy-visible — MontoyaHttpTransport buffers, matching all existing backends) + token counting + encrypted key + model selection (SC1–SC3); **native tool-use and prompt-caching deferred to a future phase** — not in SC1–SC5, recorded in 14-CONTEXT.md Deferred Ideas]*
