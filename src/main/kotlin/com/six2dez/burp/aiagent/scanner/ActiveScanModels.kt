...
object ScanPolicy {
    ...
    val AUTHZ_BYPASS_CLASSES = setOf(
        VulnClass.BFLA,
    ...
    fun isAllowedForMode(mode: ScanMode, vulnClass: VulnClass): Boolean {
    ...
    private fun pentestClasses(): Set<VulnClass> {
        return VulnClass.values().filterNot { it in PASSIVE_ONLY_VULN_CLASSES }.toSet()
    }
}
...
data class VulnContext(
    ...
    companion object {
         ...
         */
        fun fromUrl(url: String, responseBody: String = ""): VulnContext {
            val lowerUrl = url.lowercase()
            val lowerBody = responseBody.lowercase()

            return VulnContext(
                affectsAuth = listOf("/login", "/signin", "/auth", "/password", "/reset", "/oauth", "/sso", "/2fa", "/mfa", "/token")
                    .any { lowerUrl.contains(it) },
                affectsPayment = listOf("/checkout", "/payment", "/cart", "/order", "/purchase", "/billing", "/subscription", "/stripe", "/paypal")
                    .any { lowerUrl.contains(it) },
                affectsAdmin = listOf("/admin", "/dashboard", "/manage", "/control", "/settings", "/config", "/internal")
                    .any { lowerUrl.contains(it) },
                affectsPII = listOf("email", "phone", "address", "ssn", "social_security", "credit_card", "passport", "driver_license")
                    .any { lowerBody.contains(it) },
                isAPIEndpoint = lowerUrl.contains("/api/") || lowerUrl.contains("/v1/") || lowerUrl.contains("/v2/") ||
                               lowerUrl.contains("/graphql") || responseBody.trim().startsWith("{")
            )
        }
    }
}
