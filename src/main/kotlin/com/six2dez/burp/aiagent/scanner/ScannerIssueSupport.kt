...
object ScannerIssueSupport {
    ...
    fun remediation(vulnClass: VulnClass): String =
        when (vulnClass) {
            ...
            VulnClass.LFI, VulnClass.PATH_TRAVERSAL ->
                "Validate and sanitize file paths. Use allowlists for permitted files. Avoid user input in file operations."
            ...
            VulnClass.BFLA, VulnClass.BAC_HORIZONTAL, VulnClass.BAC_VERTICAL ->
                "Implement role-based access control. Verify user permissions for each action."
            ...
            VulnClass.REQUEST_SMUGGLING ->
                "Normalize or reject conflicting Content-Length/Transfer-Encoding headers. " +
                    "Use a single HTTP parser across all tiers."
            ...
            VulnClass.CORS_MISCONFIGURATION ->
                "Use explicit allowlist for origins. Never reflect arbitrary origins. Avoid wildcard with credentials."
            ...
            VulnClass.ACCOUNT_TAKEOVER ->
                "Implement secure password reset flows with short-lived tokens. " +
                    "Require email verification for email changes. Use rate limiting on auth endpoints."
            VulnClass.HOST_HEADER_INJECTION ->
                "Validate Host header against allowlist. Don't use Host header in password reset URLs or cache keys. " +
                    "Use absolute URLs with hardcoded domains."
            VulnClass.EMAIL_HEADER_INJECTION ->
                "Sanitize all email header inputs. Strip newlines and carriage returns. " +
                    "Use email libraries that handle escaping."
            VulnClass.OAUTH_MISCONFIGURATION ->
                "Strictly validate redirect_uri against exact match allowlist. " +
                    "Use state parameter with unpredictable values. Don't expose tokens in URLs."
            VulnClass.MFA_BYPASS ->
                "Implement rate limiting on MFA verification. Don't expose backup codes in responses. " +
                    "Ensure MFA cannot be skipped via direct navigation."
            VulnClass.PRICE_MANIPULATION ->
                "Validate all price/quantity calculations server-side. Never trust client-provided prices. " +
                    "Use signed carts or recalculate totals."
            VulnClass.RACE_CONDITION_TOCTOU ->
                "Use database-level locking for critical operations. Implement idempotency keys. " +
                    "Use atomic operations for balance/inventory changes."
            VulnClass.CACHE_POISONING ->
                "Don't use unkeyed headers in cached responses. Validate all header inputs. " +
                    "Use separate caches for authenticated/unauthenticated content."
            VulnClass.CACHE_DECEPTION ->
                "Don't cache responses based on URL extension alone. Use Cache-Control headers. " +
                    "Validate authentication before serving cached content."
            VulnClass.SOURCEMAP_DISCLOSURE ->
                "Don't deploy source maps to production. If needed, restrict access to authenticated users only. " +
                    "Remove sourceMappingURL comments."
            VulnClass.GIT_EXPOSURE ->
                "Block access to .git directories in web server config. Don't deploy version control directories. " +
                    "Use .gitignore and verify deployment scripts."
            VulnClass.BACKUP_DISCLOSURE ->
                "Don't store backup files in web-accessible directories. " +
                    "Configure web server to block common backup extensions. Use secure backup storage."
            VulnClass.DEBUG_EXPOSURE ->
                "Disable debug endpoints in production. Use environment-based configuration. " +
                    "Protect actuator endpoints with authentication."
            VulnClass.S3_MISCONFIGURATION ->
                "Use private bucket policies by default. Enable S3 Block Public Access. " +
                    "Audit bucket policies regularly. Use presigned URLs for temporary access."
            VulnClass.SUBDOMAIN_TAKEOVER ->
                "Remove dangling DNS records. Monitor for unclaimed resources. " +
                    "Use CNAME verification before DNS changes."
            VulnClass.API_VERSION_BYPASS ->
                "Deprecate old API versions completely. Don't leave deprecated versions accessible. " +
                    "Use consistent security across all versions."
            VulnClass.ACCESS_CONTROL_BYPASS ->
                "Don't rely on client IP headers for access control. " +
                    "Implement proper authentication and authorization. " +
                    "Use consistent access control across path variations and HTTP methods."
        }
}
